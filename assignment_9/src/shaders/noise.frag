...
float perlin_noise_1d(float x) {
	...
	*/
	 ...
	 */
	 ...
	 //gradients
	 vec2 g0 = gradients[hash_func(c0) % 12];
	 vec2 g1 = gradients[hash_func(c1) % 12];
	...
	return mix(f0, f1, blending_weight_poly(t));
...
vec3 tex_marble(vec2 point) {
	 ...
	 */
	 vec2 q = vec2(perlin_fbm(point), perlin_noise(point + vec2(1.7,4.6)));
