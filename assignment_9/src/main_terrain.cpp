...
std::shared_ptr<Mesh> build_terrain_mesh(Array2D<float> const& height_map) {
	...
	for(int gy = 0; gy < grid_size.second; gy++) {
		for(int gx = 0; gx < grid_size.first; gx++) {
			...
			double x = gx * 1.0/ grid_size.first - off;
            double y = gy * 1.0/ grid_size.second - off;

			double z = height_map(gx,gy);
			if(height_map(gx,gy) < WATER_LEVEL){
			    z = WATER_LEVEL;
			}


			int const idx = xy_to_v_index(gx, gy);
			vertices.at(idx) = vec3(x, y, z);
		}
	}
	...
	for(int gy = 0; gy < grid_size.second-1; gy++) {
		for(int gx = 0; gx < grid_size.first-1; gx++) {
			...
			Mesh::Face *f1 = new Mesh::Face(xy_to_v_index(gx,gy), xy_to_v_index(gx+1,gy), xy_to_v_index(gx,gy+1));
            Mesh::Face *f2 = new Mesh::Face(xy_to_v_index(gx+1,gy), xy_to_v_index(gx+1,gy+1), xy_to_v_index(gx,gy+1));
