...
void ShaderViewer::setShaderFiles(const std::string& vertex_shader_path, const std::string& fragment_shader_path)
{
	...
	have_time = shader_to_display.has_uniform("t");
}
...
void ShaderViewer::zoom(double amount)
{
	const double scale_min = 0.1;
	const double scale_max = 80.0;
	viewer_scale = std::min(std::max(viewer_scale * (1 + 0.1 * amount), scale_min), scale_max);
	should_redraw = true;
}
...
void ShaderViewer::mouse_button(int button, int action, int mods)
{
	if (button == GLFW_MOUSE_BUTTON_LEFT) {
		...
		if (mouse_pressed) {
			...
			glfwSetCursorPosCallback(window_, [](GLFWwindow* window, double xpos, double ypos) {
					auto *that = static_cast<ShaderViewer*>(getHandle(window));
					that->mouse_drag(vec2(xpos, ypos));
					});
		} else {
...
void ShaderViewer::mouse_drag(vec2 pos) {
	vec2 off_sc = (pos - last_mouse_pos);
	last_mouse_pos = pos;

	vec2 aspect_ratio(width_, height_);
	vec2 vs = viewer_scale * std::sqrt(2.0f) * normalize(vec2(width_, height_));

	vec2 off = vec2(-off_sc.x/width_, off_sc.y/height_) * vs;
	setViewerPosition(viewer_position + off);
}
...
void ShaderViewer::timer(double dt)
{
	...
	if (fabs(zoom_velocity) > 1e-5) {
		zoom(zoom_velocity * dt * viewer_scale);
	}
...
void ShaderViewer::paint_shader_to_texture()
{
	...
	shader_to_display.set_uniform("t", time, true);
